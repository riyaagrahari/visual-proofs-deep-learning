...
Faithful = visible article content (links, URLs, tables, references, image
links, navboxes, categories) is preserved where the HTML->Markdown converter
emits it; only scripts/styles/meta/link machinery is removed.
...
# Use the OS-native trust store (macOS keychain) so requests works behind a
# corporate TLS-inspection proxy whose CA the system trusts but certifi does
# not bundle. No global config or env vars required.
try:
    import truststore
...
USER_AGENT = "ERA V5 multilingual BPE tokenizer/1.0 (resubmission)"
